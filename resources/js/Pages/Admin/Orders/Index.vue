...
<template>
    ...
    <AdminLayout>
        ...
        <form class="mt-8 flex flex-col gap-3 lg:flex-row" @submit.prevent="applyFilters">
            <input v-model="search" type="search" placeholder="Search customer, email, order ID, reference" class="h-12 rounded-lg border border-black px-4 lg:w-96">
            <select v-model="paymentStatus" class="h-12 rounded-lg border border-black px-4">
                <option value="">All payment statuses</option>
                <option value="paid">Successful</option>
                <option value="pending">Pending/Unsuccessful</option>
                <option value="failed">Failed</option>
            </select>
            <button class="h-12 rounded-lg bg-black px-5 font-bold text-white">Filter</button>
        </form>
            ...
            <table class="w-full min-w-[980px] text-left">
                ...
                <tbody>
                    <tr v-for="order in orders.data" :key="order.id" class="border-b border-gray-200">
                        ...
                        <td class="px-4 py-4">
                            <span class="rounded-full px-3 py-1 text-xs font-bold" :class="order.payment_status === 'paid' ? 'bg-white text-black border border-black' : 'bg-black text-white'">
                        ...
                        <td class="px-4 py-4">
                            <select
                                :value="order.status"
                                class="rounded border border-black text-sm"
                        ...
                        <td class="px-4 py-4">
                            <button type="button" class="rounded border border-black px-3 py-2 text-sm font-bold" @click="selectedOrder = order">
                                View
                            </button>
                        </td>
                    </tr>
        ...
        <Modal :show="Boolean(selectedOrder)" @close="selectedOrder = null" maxWidth="2xl">
            <div v-if="selectedOrder" class="bg-white p-6 sm:p-8">
                ...
                <div class="mt-6 space-y-3">
                    <div v-for="item in items" :key="item.id" class="flex items-center gap-4 rounded-xl border border-black p-4">
